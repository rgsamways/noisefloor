...
> **PARKED for phase one, 2026-09-21.** The case-study plan below is superseded for phase one by the vendor-neutral radio-console pivot in [`docs/NOISEFLOOR-CONSOLE-HANDOFF.md`](docs/NOISEFLOOR-CONSOLE-HANDOFF.md) — read that first. Everything in this file is kept as a record of real decisions and may resume in a later phase; it is not the current build target. See D16 below for the pivot decision itself.
...
| D15 | Narrow, logged exception to D14: a `LinkCapacityChart` **stub** (fixed demo data, no `World` dependency) is added to `packages/dashboards/src/crm/` as part of the `homepage-landing` change, ahead of `world-validator` | The landing page's design (`homepage/DESIGN-NOTES.md`) calls for the hero chart to be "the real component, not a picture" — a rendered UI component, not a static image. Scoped narrowly on purpose: fixed data only, no other `crm/*` components added, explicitly commented as a stub Phase 1 replaces wholesale, so it doesn't become a first draft of the real dashboard-family work D14 deferred. |
| D16 | **The entire case-study plan (Phases 2–7 in §4, D9/D10/D14/D15's case-dashboard work) is parked for phase one**, superseded by a single vendor-neutral radio console (schema + simulation engine + UI), per `docs/NOISEFLOOR-CONSOLE-HANDOFF.md`. Case content, scoring, and the case-player are not deleted — they may resume in a later phase built on top of the console — but they are not the active line of work. Parked openspec specs are marked with a status banner; parked packages/routes get a README note; nothing is removed from the codebase. | Case-embedded dashboards couldn't be made to feel like a live radio screen without either static fidelity ("hokie" per direct feedback) or drifting toward a pixel-accurate Ubiquiti reskin (real trademark exposure, R3). A single console with its own schema and a genuine simulation engine (correlated field movement, fault time-signatures, staleness) resolves both: nothing to look "static" next to, and nothing to accidentally copy since the UI is ours by construction. See `openspec/changes/radio-console-schema/` for the first concrete step. |
